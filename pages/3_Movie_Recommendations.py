...
def get_streaming_providers(tmdb_id):
    ...
    if response.status_code == 200:
        data = response.json()
        # Get streaming providers for US market (you can modify for other regions)
...
def get_movie_recommendations(genre, age_rating, language):
    ...
    try:
        ...
        if language in ["English", "Hindi"]:
            # Stricter criteria for English and Hindi
            params.update({
                "vote_count.gte": 100,
            ...
            # More lenient criteria for regional languages
            params.update({
                "vote_count.gte": 20,  # Even lower vote count requirement
                "vote_average.gte": 3.0,  # Lower rating requirement
                "certification_country": "IN",  # Use Indian certification
                "sort_by": "release_date.desc"  # Show newer movies first
            })
        
        # For kids (age ≤ 10), force animation genre
        if age_rating <= 10:
        ...
        if response.status_code == 200:
            ...
            # If we've reached the last page, reset to page 1
            ...
            for movie in movies:
                ...
                # For kids, ensure it's an animated movie
                if age_rating <= 10:
                    if 16 not in movie.get("genre_ids", []):  # Check if it's an animated movie
                        continue
                
                # Get detailed movie info including IMDb ID
        ...
        st.error(f"Error in movie recommendations: {str(e)}")
        return []
